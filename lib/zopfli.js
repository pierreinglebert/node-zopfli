...
Zopfli.prototype._transform = function (chunk, encoding, done) {
  ...
  var transform = this;
  zopfli.compress(new Buffer(chunk), this.format, this.options, function(err, outbuf) {
    if (err) {
      done(err);
    } else {
...
Zopfli.prototype._flush = function (done) {
  ...
  if(this.format == "zlib") {
    ...
    endBuffer.writeUInt8((parseInt(this.adler / 16777216) % 256), 0);
    endBuffer.writeUInt8((parseInt(this.adler / 65536) % 256), 1);
    endBuffer.writeUInt8((parseInt(this.adler / 256) % 256), 2);
    ...
    endBuffer.writeUInt8(parseInt(this.insize / 256) % 256, 5);
    endBuffer.writeUInt8(parseInt(this.insize / 65536) % 256, 6);
    endBuffer.writeUInt8(parseInt(this.insize / 16777216) % 256, 7);
  }
  this.push(endBuffer);
  done();
};
